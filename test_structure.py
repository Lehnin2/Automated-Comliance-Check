...
def validate_document(document_json, rules_json, metadata_json, api_key):
    ...
    http_client = httpx.Client(verify=False)  # Désactive la vérification SSL
    client = OpenAI(
        api_key=api_key,
...
Be systematic and check EVERY rule - do not skip any."""
    ...
    print("🔍 Analyzing document compliance using TokenFactory API (Llama-3.1-70B)...")
...
def main():
    ...
    if not api_key:
        print("\n❌ Error: TOKENFACTORY_API_KEY not found in .env file")
    ...
    print(f"🌐 Using TokenFactory endpoint: https://tokenfactory.esprit.tn/api")
    print(f"🤖 Model: Llama-3.1-70B-Instruct\n")
    print("="*80)
