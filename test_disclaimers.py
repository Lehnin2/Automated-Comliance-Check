...
class DisclaimerComplianceChecker:
    ...
    def __init__(self, api_key: str):
        """Initialize the compliance checker with API credentials"""
    ...
    def step3_extract_document_disclaimers(self) -> Dict[str, List[str]]:
        ...
        if 'page_de_fin' in self.document:
            legal_text = self.document['page_de_fin'].get('content', {}).get('legal_mention_sgp', {}).get('text', '')
            if legal_text:
        ...
        if 'pages_suivantes' in self.document:
            for slide in self.document['pages_suivantes']:
                disclaimers = slide.get('content', {}).get('disclaimers', [])
        ...
        if 'page_de_fin' in self.document:
            add_disclaimers = self.document['page_de_fin'].get('content', {}).get('additional_disclaimers', [])
...
"""
        ...
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": "You are a compliance analyst expert. Always respond with valid JSON."},
    ...
    def step7_consistency_check(self) -> List[Dict[str, Any]]:
        ...
        if 'belgium' in all_text:
            ...
            if 'page_de_fin' in self.document:
                additional_text = self.document['page_de_fin'].get('content', {}).get('additional_text', '').lower()
...
def main():
    ...
    # Initialize checker
    checker = DisclaimerComplianceChecker(api_key)
