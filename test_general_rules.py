...
class ComplianceAnalyzer:
    ...
    def __init__(self, api_key: str = None):
        """Initialize the analyzer with Token Factory API."""
        self.api_key = api_key or os.environ.get('TOKEN_FACTORY_API_KEY')
        if not self.api_key:
            raise ValueError("TOKEN_FACTORY_API_KEY not found in environment variables")
        
        # Create HTTP client with SSL verification disabled
        http_client = httpx.Client(verify=False)
        
        # Initialize OpenAI client with Token Factory endpoint
        self.client = OpenAI(
    ...
    def _call_llm(self, prompt: str, max_tokens: int = 19500) -> str:
        """Helper method to call the LLM with consistent parameters."""
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[
                {"role": "system", "content": "You are a compliance analysis expert. You analyze documents for regulatory compliance and provide detailed, structured responses."},
...
def main():
    ...
    try:
        ...
        # Save report to file
        output_file = f"compliance_report_{document_path.replace('.json', '')}.md"
